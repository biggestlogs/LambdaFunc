...
longest_name_student = max(students, key=lambda s: len(s["name"]))
...
by_rating_desc = sorted(products, key=lambda x: x["rating"], reverse=True)

# E3) TODO: Use map + lambda to create a list of product names only
names = list(map(lambda x: x["name"], by_price))

# E4) TODO: Use filter + lambda to keep only products with rating >= 4.5
top_rated = list(filter(lambda x: x["name"] in names, by_rating_desc))

# E5) TODO: Combine ideas: get the NAME of the highest-rated product
best_name = list(filter(lambda x: x["name"] in names, by_rating_desc))
